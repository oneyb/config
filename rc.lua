...
local datewidget = awful.widget.textclock({ align = "right" })
local calendar2 = require('calendar2')
calendar2.addCalendarToWidget(datewidget)

-- datewidget = awful.widget.textclock({ align = "right" })
-- local datewidget = wibox.widget.calendar.month(os.date('*t'))

-- Volume
local sink = "alsa_output.pci-0000_00_1b.0.analog-stereo"
-- local volumewidget = wibox.widget({ type = "textbox"})
...
-- local telegramwidget = wibox.widget.imagebox()
-- telegramwidget.image = awful.util.get_configuration_dir() .. "/icons/telegram-icon.png"
-- telegramwidget:buttons(awful.util.table.join(
-- 		      awful.button({ }, 1, function () awful.spawn(os.getenv("HOME") .. "/bin/tele") end)
-- ))
...
local whatsappwidget = wibox.widget.imagebox()
whatsappwidget.image = awful.util.get_configuration_dir() .. "/icons/face-monkey.png"
whatsappwidget:buttons(awful.util.table.join(
                         awful.button({ }, 1, function () awful.spawn(os.getenv("HOME") .. "/bin/whatsapp") end)
))
...
local fbmesswidget = wibox.widget.imagebox()
fbmesswidget.image = awful.util.get_configuration_dir() .. "/icons/messengerfordesktop.png"
fbmesswidget:buttons(awful.util.table.join(
                       awful.button({ }, 1, function () awful.spawn(os.getenv("HOME") .. "/bin/messy") end)
